...
// SteamRIP code copy button
document.addEventListener("click", async (event) => {
  const button = event.target.closest(".steamrip-copy-btn");
  if (!button) return;
  ...
  const wrapper = button.closest(
    ".steamrip-code-wrap, .code-block-wrap, .code-container"
  );

  const code = wrapper?.querySelector("pre code, pre.steamrip-code, textarea");
  if (!code) return;
